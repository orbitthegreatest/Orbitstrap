...
namespace Orbitstrap
{
    internal class Installer
    {
        ...
        /// <summary>
        /// Flushes the Windows shell icon cache globally so Explorer, the desktop
        /// shortcut, and the taskbar all pick up the new logo immediately.
        /// </summary>
        internal static void RefreshShellIcons()
        {
            ...
            catch (Exception ex) { App.Logger.WriteException("Installer::RefreshShellIcons", ex); }
        }

        /// <summary>
        /// Copies every file and sub-directory from <paramref name="sourceDir"/> into
        /// <paramref name="destDir"/>, overwriting existing files.
        /// Required for Debug / non-single-file builds that ship companion DLLs,
        /// runtimeconfig.json, and native sub-folders next to the exe.
        /// For single-file published builds the source folder only contains the exe,
        /// so the behaviour is identical to a plain File.Copy.
        /// </summary>
        private static void CopyDirectory(string sourceDir, string destDir)
        {
            ...
            foreach (string srcSubDir in Directory.GetDirectories(sourceDir))
                CopyDirectory(srcSubDir, Path.Combine(destDir, Path.GetFileName(srcSubDir)));
        }

        /// <summary>
        /// Force-recreates the desktop and Start-menu shortcuts so they pick up the
        /// current exe icon instead of keeping a stale cached version.
        /// </summary>
        private static void RefreshShortcuts()
        {
            foreach (string lnkPath in new[] { DesktopShortcut, StartMenuShortcut })
            {
                try
                {
                ...
                {
                    App.Logger.WriteException("Installer::RefreshShortcuts", ex);
                }
            }
        }
        ...
        public bool OrbitstrapRPCReal = true;
        ...
        public void DoInstall()
        {
            ...
            if (!IsImplicitInstall)
            {
                ...
                try
                {
                    // Only copy the executable — NOT the entire source folder.
                    // The old CopyDirectory call was duplicating every file next to
                    // the installer into %LocalAppData%\Orbitstrap.
                    string destExe = Path.Combine(Paths.Base, Path.GetFileName(Paths.Process));
                    App.Logger.WriteLine(LOG_IDENT, $"Copying exe to \"{destExe}\"");
                    File.Copy(Paths.Process, destExe, overwrite: true);
                }
                catch (Exception ex)
                {
            ...
            // only register player, for the scenario where the user installs Orbitstrap, closes it,
        ...
        public static void HandleUpgrade()
        {
            ...
            for (int i = 1; i <= 10; i++)
            {
                try
                {
                    // Only copy the executable for updates too.
                    string destExe = Path.Combine(Paths.Base, Path.GetFileName(Paths.Process));
                    File.Copy(Paths.Process, destExe, overwrite: true);
                    break;
                }
            ...
            // Rebuild shortcuts so they reflect the new exe icon, then flush the
            // Windows shell icon cache so Explorer and the desktop show the new logo.
            RefreshShortcuts();
            RefreshShellIcons();
            ...
            if (existingVer is not null)
            {
                ...
                if (Utilities.CompareVersions(existingVer, "1.0.3.6") == VersionComparison.LessThan)
                {
                    ...
                    string oldStartPath = Path.Combine(Paths.WindowsStartMenu, "Orbitstrap");
                    ...
                    Registry.CurrentUser.DeleteSubKeyTree("Software\\Orbitstrap", false);
...
#pragma warning disable CS0162 // Unreachable code detected
                if (OpenReleaseNotes)
                    Utilities.ShellExecute($"https://github.com/{App.ProjectRepository}/wiki/Release-notes-for-Orbitstrap-v{currentVer}");
