...
namespace Orbitstrap.UI.Elements.Base
{
    ...
    public abstract class WpfUiWindow : UiWindow, IDisposable
    {
        ...
        public void ApplyTheme()
        {
            ...
            // Only follow the Windows OS accent color when the user has explicitly
            // selected the "Custom" theme. Previously this was called unconditionally,
            // which silently overwrote every built-in theme's own accent color (e.g.
            // Orbitstrap red) with the OS accent color -- causing the window glow and
            // hyperlinks to appear purple on systems with a purple Windows accent,
            // regardless of the theme actually selected in-app.
            if (isCustom)
            ...
            if (isCustom)
            {
                ...
                if (!File.Exists(customXshdPath))
                {
                    _ = Task.Run(async () =>
                    {
                        var url = "https://raw.githubusercontent.com/KloBraticc/OrbitstrapCustomThemes/main/Editor-Theme-Custom.xshd";
