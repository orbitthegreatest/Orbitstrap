...
using AccountManagerService = Orbitstrap.Integrations.AccountManager;
using Orbitstrap.Integrations;
using AccountManagerWindow = Orbitstrap.UI.Elements.AccountManager.MainWindow;
...
namespace Orbitstrap.UI.Elements.Settings
{
    public partial class MainWindow : INavigationWindow
    {
        ...
        private void AccountManagerNavItem_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var w = new AccountManagerWindow();
                w.Owner = this;
                w.Show();
            }
            catch (Exception ex)
            {
                App.Logger.WriteException("MainWindow::AccountManagerNavItem_Click", ex);
                Frontend.ShowMessageBox($"Failed to open Account Manager:\n{ex.Message}", System.Windows.MessageBoxImage.Error);
            }
        }

        private void AccountHeader_Click(object sender, MouseButtonEventArgs e)
        {
            AccountManagerNavItem_Click(sender, e);
        }

        private void OnActiveAccountChanged(AltAccount? account)
        {
            Dispatcher.Invoke(() => RefreshAccountHeader(account));
        }

        private void RefreshAccountHeader(AltAccount? account)
        {
            if (AccountUsernameText == null)
                return; // XAML not yet initialized

            if (account == null)
            {
                AccountUsernameText.Text = "Sign In";
                AccountSubtitleText.Text = "Click to sign in";
                AccountAvatarImage.Visibility = Visibility.Collapsed;
                AccountAvatarFallback.Visibility = Visibility.Visible;
                AccountAvatarSymbol.Visibility = Visibility.Visible;
                return;
            }

            AccountUsernameText.Text = string.IsNullOrEmpty(account.DisplayName) ? account.Username : account.DisplayName;
            AccountSubtitleText.Text = $"@{account.Username}";

            _ = LoadAccountAvatarAsync(account.UserId);
        }

        private async Task LoadAccountAvatarAsync(long userId)
        {
            const string LOG_IDENT = "MainWindow::LoadAccountAvatarAsync";

            try
            {
                var url = $"https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds={userId}&size=48x48&format=Png&isCircular=false";
                ...
                var imageUrl = dataArray[0].GetProperty("imageUrl").GetString();
                if (string.IsNullOrEmpty(imageUrl))
                    return;

                var imageBytes = await App.HttpClient.GetByteArrayAsync(imageUrl);
                ...
                await Dispatcher.InvokeAsync(() =>
                {
                    ...
                    AccountAvatarSymbol.Visibility = Visibility.Collapsed;
                });
            }
            catch (Exception ex)
            {
                // Avatar fetch failing (e.g. no internet, rate limit) isn't fatal -- the
                // fallback person icon just stays visible. Log and move on.
                App.Logger.WriteLine(LOG_IDENT, "Failed to load account avatar");
                ...
                public MainWindow(bool showAlreadyRunningWarning)
        {
            ...
            Closed += (s, e) => AccountManagerService.Shared.ActiveAccountChanged -= OnActiveAccountChanged;

            AccountManagerService.Shared.ActiveAccountChanged += OnActiveAccountChanged;
            RefreshAccountHeader(AccountManagerService.Shared.ActiveAccount);
        ...
        private void OpenToolbox(TabItemViewModel targetTab, MainWindowViewModel vm)
        {
            ...
            CreateToolItem("Background Snow", "Adds Snow to Orbitstraps background (Restart Required)");
        ...
        private void UpdateDiscordPresence()
        {
            ...
            {
                Details = $"Viewing {pageName}", // the fuck was there state I just relized that it already displays fucking orbitstrap THE FUCK
            ...
            new DiscordRPC.Button
            {
                ...
                Url = "https://github.com/orbitstrap/Orbitstrap"
            }
        }
            });
        }
        ...
        {
            // if (FastFlagEditorNavItem == null) // Removed: FastFlag Editor nav item deleted
                return;

            var shouldBeVisible = !App.Settings.Prop.LockDefault;
            // if (Visibility.Collapsed // FastFlagEditorNavItem removed == (shouldBeVisible ? Visibility.Visible : Visibility.Collapsed)) // Removed: FastFlag Editor nav item deleted
                return;

            // Visibility.Collapsed // FastFlagEditorNavItem removed = shouldBeVisible ? Visibility.Visible : Visibility.Collapsed; // Removed: FastFlag Editor nav item deleted
        }
