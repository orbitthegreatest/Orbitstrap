...
namespace Orbitstrap.UI.Elements.Settings.Pages
{
    public partial class ChannelPage
    {
        ...
        private void AddResolutionRule_Click(object sender, RoutedEventArgs e)
        {
        ...
        private void ApplyNow_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                string modsSource = Path.Combine(basePath, "OrbitstrapMods");

                var strapDirs = Directory.GetDirectories(localAppData)
                    .Where(d => d.EndsWith("strap", StringComparison.OrdinalIgnoreCase) &&
                                !d.EndsWith("Orbitstrap", StringComparison.OrdinalIgnoreCase))
                    .ToList();
                ...
                Frontend.ShowMessageBox("Orbitstrap Settings/Mods Synced"); // diddy is synced :)))))))))) eheh not funny ik.. sorry
            }
            catch (Exception ex)
            {
        ...
        private async void Check_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    Frontend.ShowMessageBox(
                        "You are already running the latest version of Orbitstrap."
                    );
                }
            }
            catch (Exception ex)
