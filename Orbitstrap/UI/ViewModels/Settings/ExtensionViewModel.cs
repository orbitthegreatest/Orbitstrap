...
namespace Orbitstrap.UI.Elements.Settings.Pages
{
    public class ExtensionViewModel
    {
        ...
        // Fleasion moved from the old qrhrqiohj/Fleasion account to the fleasion/Fleasion
        // org, and release artifacts are no longer a fixed "Fleasion.exe" name (they're
        // versioned per-platform, e.g. "Fleasion-v2.3.0-Windows.exe"). Ask the GitHub API
        // for the latest release and pick out the Windows .exe asset so this keeps working
        // even if the exact filename changes between releases.
        private static async Task<string> GetLatestFleasionWindowsAssetUrl(CancellationToken ct)
        {
            const string apiUrl = "https://api.github.com/repos/fleasion/Fleasion/releases/latest";

            using HttpRequestMessage request = new(HttpMethod.Get, apiUrl);
            request.Headers.UserAgent.ParseAdd("Orbitstrap");
            ...
            if (!doc.RootElement.TryGetProperty("assets", out JsonElement assets))
                throw new InvalidOperationException("Fleasion release response did not contain any assets.");
            ...
            foreach (JsonElement asset in assets.EnumerateArray())
            {
                    ...
                    !name.Contains("macOS", StringComparison.OrdinalIgnoreCase))
                {
                    return asset.GetProperty("browser_download_url").GetString()
                        ?? throw new InvalidOperationException("Fleasion Windows asset had no download URL.");
                }
            }

            throw new InvalidOperationException("Could not find a Windows .exe asset in the latest Fleasion release.");
        }
        ...
        private async Task DownloadFleasion()
        {
            ...
            try
            {
                ...
                client.DefaultRequestHeaders.UserAgent.ParseAdd("Orbitstrap");

                string downloadUrl = await GetLatestFleasionWindowsAssetUrl(ct);
