...
using static Orbitstrap.UI.Elements.Settings.Pages.ModsPage;
...
namespace Orbitstrap.UI.ViewModels.Settings
{
    public class ModsViewModel : NotifyPropertyChangedViewModel
    {
        ...
        public bool UseDarkTextureMod
        {
            get => App.Settings.Prop.UseDarkTextureMod;
            set
            {
        ...
        public bool UseBlackCursorMod
        {
            get => App.Settings.Prop.UseBlackCursorMod;
            set
            {
                App.Settings.Prop.UseBlackCursorMod = value;
                OnPropertyChanged();

                try
                {
                    if (value)
                        BlackCursorMod.Apply();
                    else
                        BlackCursorMod.Remove();
                }
                catch (Exception ex)
                {
        ...
        public bool UseKorbloxRightLeg
        {
            get => App.Settings.Prop.UseKorbloxRightLeg;
            set
            {
                App.Settings.Prop.UseKorbloxRightLeg = value;
                OnPropertyChanged();
                App.Settings.Save();
            }
        }

        public bool UseHeadlessMod
        {
            get => App.Settings.Prop.UseHeadlessMod;
            set
            {
                App.Settings.Prop.UseHeadlessMod = value;
                OnPropertyChanged();
                App.Settings.Save();
            }
        }

        public ObservableCollection<EmoteWheelMod.ManifestEntry> EmoteWheelOptions { get; } = new();

        public async Task LoadEmoteWheelOptionsAsync()
        {
            try
            {
                EmoteWheelOptions.Clear();
                EmoteWheelOptions.Add(new EmoteWheelMod.ManifestEntry("None", "None (Default)", ""));

                var entries = await EmoteWheelMod.GetManifestAsync();
                foreach (var entry in entries)
                    EmoteWheelOptions.Add(entry);
            }
            catch (Exception ex)
            {
                App.Logger?.WriteException("ModsViewModel::LoadEmoteWheelOptionsAsync", ex);
            }
            finally
            {
                // This is kicked off fire-and-forget from the constructor, so the ComboBox's
                // SelectedValue binding can (and, depending on how long the manifest fetch takes,
                // randomly does) get evaluated against EmoteWheelOptions while it's still empty.
                // When that happens WPF resolves it to "no selection" and the box shows blank,
                // even though SelectedEmoteWheelId / App.Settings.Prop.SelectedEmoteWheel still
                // hold the correct id (which is why opening the dropdown still works/shows the
                // right item checked). Re-raise the change now that the options are actually
                // populated so the box re-resolves its displayed value against them.
                OnPropertyChanged(nameof(SelectedEmoteWheelId));
            }
        }

        // Guards ApplyAsync/Remove against overlapping calls: if the user changes the dropdown
        // again before a previous apply has finished downloading/extracting, both calls used to
        // race on the exact same fixed temp file names in EmoteWheelMod (orbitstrap-emotewheel.zip /
        // orbitstrap-emotewheel-extract), which could interleave and leave the WRONG wheel's files
        // as the ones actually copied into the Mods folder — e.g. selecting "Miguel" right after
        // load could still result in "Cute Bears" being the one that lands on disk, because the
        // two Task.Run calls were stepping on each other's temp files. The lock below serializes
        // apply calls, and the generation counter makes an in-flight apply for a stale selection
        // bail out (without touching disk) once a newer selection has already come in.
        private static readonly System.Threading.SemaphoreSlim _emoteWheelApplyLock = new(1, 1);
        private int _emoteWheelApplyGeneration;

        private string _selectedEmoteWheelId = App.Settings.Prop.SelectedEmoteWheel;
        public string SelectedEmoteWheelId
        {
            get => _selectedEmoteWheelId;
            set
            {
                if (_selectedEmoteWheelId == value)
                    return;

                _selectedEmoteWheelId = value;
                App.Settings.Prop.SelectedEmoteWheel = value;
                OnPropertyChanged();

                int myGeneration = System.Threading.Interlocked.Increment(ref _emoteWheelApplyGeneration);
                ...
                _ = Task.Run(async () =>
                {
                    ...
                    try
                    {
                        ...
                        {
                            EmoteWheelMod.Remove();
                        }
                        else
                        {
                            var entry = EmoteWheelOptions.FirstOrDefault(e => e.Id == value);
                            if (entry is not null)
                                await EmoteWheelMod.ApplyAsync(entry.Url);
                        }
                    }
                    catch (Exception ex)
                    {
                        App.Logger.WriteException("ModsViewModel::SelectedEmoteWheelId", ex);
                        System.Windows.Application.Current.Dispatcher.Invoke(() =>
                            Frontend.ShowMessageBox(
                                $"Failed to apply the Emote Wheel Mod:\n{ex.Message}",
                                System.Windows.MessageBoxImage.Warning
                            )
                        );
                    }
        ...
        public async Task LoadModsAsync() // was working on this but idk fuc
        {
            ...
            http.DefaultRequestHeaders.UserAgent.ParseAdd("OrbitstrapApp");
        ...
        // Skybox source — switched from the KloBraticc/SkyboxPackV2 GitHub API listing to
        // Orbitstrap's own external assets repo (manifest.json + per-skybox zip, same pattern
        // Task 2B's emote wheel selector uses). Actual download/extraction happens later, at
        // launch time, in Bootstrapper.EnsureSkyboxDownloadedAsync/ApplySkyboxAsync — this only
        // needs the id/name list to populate the dropdown.
        ...
        public async Task LoadSkyboxPacksFromGithub()
        {
            ...
            List<EmoteWheelMod.ManifestEntry> manifest;
            try
            {
                manifest = await Orbitstrap.Utility.Http.GetJson<List<EmoteWheelMod.ManifestEntry>>(SkyboxManifestUrl);
            }
            catch (Exception ex)
            {
                App.Logger?.WriteException("ModsViewModel::LoadSkyboxPacksFromGithub", ex);
                return;
            }

            foreach (var entry in manifest)
            {
                AvailableSkyboxPacks.Add(new SkyboxPack { Id = entry.Id, Name = entry.Name });
            }

            if (AvailableSkyboxPacks.Count == 0)
                return;

            var selected = AvailableSkyboxPacks.FirstOrDefault(s =>
                s.Id.Equals(App.Settings.Prop.SkyboxName, StringComparison.OrdinalIgnoreCase))
                ?? AvailableSkyboxPacks.First();

            SelectedSkyboxPack = selected;
            App.Settings.Prop.SkyboxName = selected.Id;
        }
        ...
        public SkyboxPack? SelectedSkyboxPack
        {
            ...
            set
            {
                if (_selectedSkyboxPack != value)
                {
                    ...
                    {
                        App.Settings.Prop.SkyboxName = _selectedSkyboxPack.Id;
                    }
                }
            }
        }

        public new ICommand OpenModsFolderCommand => new RelayCommand(OpenModsFolder);
        ...
        public async Task ApplyAllPendingPresetsAsync()
        {
            if (SelectedCursorPreset != null)
            {
                ...
                {
                    try { await CursorPresetMod.ApplyDefault(); }
            ...
            if (SelectedFontPreset != null)
            {
                ...
                {
                    try { FontPresetMod.Remove(); await FontPresetMod.ApplyDefault(); }
            ...
            if (SelectedDeathSoundPreset != null)
            {
                ...
                {
                    try { DeathSoundPresetMod.Remove(); await DeathSoundPresetMod.ApplyDefault(); }
    ...
    GetVisibility(Path.Combine(Paths.Mods, "Content", "textures", "Cursors", "KeyboardMouse"),
        ...
        public void RemoveCustomCursorMod()
        {
            ...
            RemoveCustomFile(
                ...
                () =>
                {
                    ...
                    _ = CursorPresetMod.ApplyDefault();
        ...
        public void RemoveCustomDeathSound()
        {
            RemoveCustomFile(
                ...
                () =>
                {
                    ...
                    _ = DeathSoundPresetMod.ApplyDefault();
        ...
        private readonly string _dir =
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Orbitstrap");
