...
namespace Orbitstrap.UI.ViewModels.Settings
{
    public class ChannelViewModel : INotifyPropertyChanged
    {
        ...
        // ---- Advanced: multiple games with different in-game resolutions ----

        public ObservableCollection<GameResolutionRule> GameResolutionRules =>
            App.Settings.Prop.GameResolutionRules;

        private string _newRuleName = "";
        public string NewRuleName
        {
            get => _newRuleName;
            set { _newRuleName = value; OnPropertyChanged(nameof(NewRuleName)); }
        }

        private string _newRulePlaceId = "";
        public string NewRulePlaceId
        {
            get => _newRulePlaceId;
            set { _newRulePlaceId = value; OnPropertyChanged(nameof(NewRulePlaceId)); }
        }

        private DisplayMode? _newRuleResolution;
        public DisplayMode? NewRuleResolution
        {
            get => _newRuleResolution;
            set { _newRuleResolution = value; OnPropertyChanged(nameof(NewRuleResolution)); }
        }

        public void AddGameResolutionRule()
        {
            if (string.IsNullOrWhiteSpace(NewRulePlaceId) || NewRuleResolution is null)
            {
                MessageBox.Show("Enter a Place ID and pick a resolution before adding a rule.", "Missing information", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }

            if (!long.TryParse(NewRulePlaceId.Trim(), out _))
            {
                MessageBox.Show("Place ID must be a number.", "Invalid Place ID", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }

            GameResolutionRules.Add(new GameResolutionRule
            {
                Name = string.IsNullOrWhiteSpace(NewRuleName) ? $"Place {NewRulePlaceId.Trim()}" : NewRuleName.Trim(),
                PlaceId = NewRulePlaceId.Trim(),
                MatchUniverseId = false,
                Resolution = new ResolutionSetting
                {
                    Width = NewRuleResolution.Width,
                    Height = NewRuleResolution.Height,
                    RefreshRate = NewRuleResolution.RefreshRate
                }
            });
        ...
        public bool ForceRobloxReinstallation
        {
            ...
            set
            {
                App.Settings.Prop.ForceRobloxReinstallation = value;
                App.Settings.Save();
                OnPropertyChanged(nameof(ForceRobloxReinstallation));
            }
        }

        public bool SkipOrbitstrapUpdate
        {
            get => App.Settings.Prop.SkipOrbitstrapUpdate;
            set
            {
                if (App.Settings.Prop.SkipOrbitstrapUpdate != value)
                {
                    App.Settings.Prop.SkipOrbitstrapUpdate = value;
                    OnPropertyChanged(nameof(SkipOrbitstrapUpdate));
                    App.Settings.Save();
                }
            }
        }
