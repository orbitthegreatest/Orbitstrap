    ...
    Task.Run(() =>
    {
        ...
        private void UpdateRpcPresence(bool force = false)
        {
            ...
            if (NowPlaying == null || string.IsNullOrWhiteSpace(NowPlaying.Title))
            {
                ...
                {
                    Details = "Idle",
                    State = "Orbitstrap Music Player",
                    Assets = new DRPC.Assets
                    {
                        LargeImageKey = "orbitstrap_logo",
                        LargeImageText = "Orbitstrap Music Player"
                    }
                };
            }
            else
            {
                ...
                string infoString = $"{(NowPlaying.FileType ?? "FILE").ToUpperInvariant()} • {elapsedStr} / {totalStr} • {(_isPlaying ? "Playing" : "Paused")} in Orbitstrap{loopText}";
