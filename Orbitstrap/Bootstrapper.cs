...
using Strings = Orbitstrap.Resources.Strings;
...
using static Orbitstrap.UI.ViewModels.Settings.ModsViewModel;
...
namespace Orbitstrap
{
    public class Bootstrapper
    {
        ...
        // Skybox source repo — switched from KloBraticc/SkyboxPackV2 to Orbitstrap's own
        // external assets repo (per-skybox zips + manifest.json, same pattern EmoteWheelMod uses).
        private const string SkyboxManifestUrl =
            "https://raw.githubusercontent.com/orbitthegreatest/Orbitstrap-things/main/skyboxes/manifest.json";
        ...
        public async Task Run()
        {
            ...
            if (!App.LaunchSettings.UpgradeFlag.Active && !App.Settings.Prop.SkipOrbitstrapUpdate)
            ...
            try
            {
                using (Mutex.OpenExisting("Orbitstrap-Bootstrapper"))
                {
                    App.Logger.WriteLine(LOG_IDENT, "Orbitstrap-Bootstrapper mutex exists, waiting...");
            ...
            await using var mutex = new AsyncMutex(false, "Orbitstrap-Bootstrapper");
            ...
            if (!_noConnection && App.Settings.Prop.ForceRobloxReinstallation)
            {
                App.Logger.WriteLine(LOG_IDENT, "Force Roblox reinstallation is enabled, clearing cached version to force a fresh install.");

                // clearing the stored version guid makes _mustUpgrade true, so UpgradeRoblox()
                // below will delete whatever is currently installed at this version and
                // redownload it from scratch, even if it's already the latest version.
                AppData.State.VersionGuid = "";

                // one-shot: turn the toggle back off now that the reinstall has been queued,
                // so it doesn't keep forcing a fresh install on every future launch.
                App.Settings.Prop.ForceRobloxReinstallation = false;
                App.Settings.Save();
            }
        ...
        private async Task CheckAndApplyUpdate(string logIdent)
        {
            ...
            if (ok)
            {
                App.Logger.WriteLine(logIdent, "Update installed restarting Orbitstrap...");
                RestartApplication();
            }
            else
            {
        ...
        private static async Task<T> RetryOnOverloadAsync<T>(
            Func<Task<T>> action,
            string logIdent,
            int maxAttempts = 5,
            int baseDelayMs = 1500)
        {
            ...
            for (int attempt = 1; attempt <= maxAttempts; attempt++)
            {
                    ...
                    ex.StatusCode == HttpStatusCode.TooManyRequests))
                {
                    ...
                    int jitter = (int)(delay * 0.2 * (rng.NextDouble() * 2 - 1));
                    delay = Math.Clamp(delay + jitter, 500, 30_000);
                    App.Logger.WriteLine(logIdent,
                        $"Overload ({(int)(ex.StatusCode ?? 0)}) attempt {attempt}/{maxAttempts}, retrying in {delay}ms...");
                ...
                private async Task GetLatestVersionInfo()
        {
                ...
                infoUrl, HttpCompletionOption.ResponseHeadersRead).ConfigureAwait(false))
            {
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    else if (response.StatusCode == (HttpStatusCode)529 ||
                             response.StatusCode == HttpStatusCode.ServiceUnavailable ||
                             response.StatusCode == HttpStatusCode.BadGateway ||
                             response.StatusCode == HttpStatusCode.GatewayTimeout ||
                             response.StatusCode == HttpStatusCode.TooManyRequests)
                    {
                        // Transient server-side overload — retry with backoff
                        App.Logger.WriteLine(LOG_IDENT, $"Transient {(int)response.StatusCode} — will retry via Deployment.GetInfo.");
                        clientVersion = await RetryOnOverloadAsync(
                            () => Deployment.GetInfo(Deployment.Channel),
                            LOG_IDENT).ConfigureAwait(false);
                    }
                    else
                    {
        ...
        private void RestartMemoryCleanerFromSettings()
        {
            ...
            var settings = OrbitstrapRobloxSettingsManager.Load();
        ...
        private void ExtractPackage(Package package, List<string>? files = null)
        {
            ...
            string destRoot = Path.Combine(_latestVersionDirectory, packageDir);
            ...
            if (files is not null)
            {
                ...
                // FastZip overwrites destination files with a raw write and does NOT clear a
                // pre-existing read-only attribute first (unlike the mod-copy loop above, which
                // calls Filesystem.AssertReadOnly before every File.Copy). If one of these files
                // is currently read-only — e.g. a font-family json that used to be a mod file and
                // is now being restored to its packaged default — ExtractZip throws
                // UnauthorizedAccessException instead of overwriting it. Clear the flag up front
                // for every file we're about to restore.
                foreach (string relFile in files)
                    Filesystem.AssertReadOnly(Path.Combine(destRoot, relFile));
            }
            else if (Directory.Exists(destRoot))
            {
                Filesystem.AssertReadOnlyDirectory(destRoot);
            }

            App.Logger.WriteLine(LOG_IDENT, $"Extracting {package.Name}...");
            new FastZip(_fastZipEvents)
                .ExtractZip(package.DownloadPath, destRoot, fileFilter);
        ...
        private async Task ApplyModifications()
        {
            ...
            foreach (string file in Directory.GetFiles(Paths.Mods, "*.*", SearchOption.AllDirectories))
            {
                ...
                // EmoteWheelMod's own bookkeeping file (tracks which files it wrote so it can
            ...
            try
            {
                if (App.Settings.Prop.UseKorbloxRightLeg)
                {
                    App.Logger.WriteLine(LOG_IDENT, "Applying Korblox right-leg mesh…");
                    await Utility.KorbloxHeadlessMod.ApplyKorbloxRightLegAsync(_latestVersionDirectory);
                }
                else
                {
            ...
            {
                App.Logger.WriteException(LOG_IDENT + "::KorbloxRightLeg", ex);
            }

            // ── Headless ─────────────────────────────────────────────────────────
            try
            {
                if (App.Settings.Prop.UseHeadlessMod)
                {
                    App.Logger.WriteLine(LOG_IDENT, "Applying Headless mod…");
                    Utility.KorbloxHeadlessMod.ApplyHeadless(_latestVersionDirectory);
                }
                else
                {
                    // Restore whenever the heads folder is currently empty (or missing) — that's
                    // the actual signal that Headless was applied and never reverted.
                    //
                    // The previous check gated this on the *default-heads cache* existing, but
                    // that cache is only ever created as a side effect of this same
                    // RevertHeadlessAsync call — it's never populated by ApplyHeadless. So on a
                    // user's very first "uncheck Headless", the cache didn't exist yet, the
                    // condition was false, and the (already-emptied) heads folder was never
                    // restored — Headless looked stuck on permanently.
                    string headsDir = Path.Combine(_latestVersionDirectory, "content", "avatar", "heads");
                    bool headsEmpty = !Directory.Exists(headsDir) || !Directory.EnumerateFiles(headsDir).Any();
                    if (headsEmpty)
            ...
            {
                App.Logger.WriteException(LOG_IDENT + "::Headless", ex);
            }
        ...
        /// <summary>Downloads and caches (by manifest id) the selected skybox from the
        /// Orbitstrap-things repo's skyboxes/manifest.json, mirroring the manifest-driven
        /// pattern EmoteWheelMod already uses for emote wheels. Returns the local cache folder
        /// for that skybox. If it's already cached locally, no network request is made.</summary>
        public static async Task<string> EnsureSkyboxDownloadedAsync(string skyboxId)
        {
            Directory.CreateDirectory(PackFolder);
            string skyboxDir = Path.Combine(PackFolder, skyboxId);

            if (Directory.Exists(skyboxDir) &&
                Directory.GetFiles(skyboxDir, "*", SearchOption.AllDirectories).Length > 0)
                return skyboxDir;

            App.Logger.WriteLine("Bootstrapper::EnsureSkyboxDownloadedAsync", "Downloading Skybox...");

            var manifest = await Orbitstrap.Utility.Http.GetJson<List<EmoteWheelMod.ManifestEntry>>(SkyboxManifestUrl);
            var entry = manifest.FirstOrDefault(e => e.Id.Equals(skyboxId, StringComparison.OrdinalIgnoreCase));
            if (entry is null)
                throw new InvalidOperationException($"Skybox '{skyboxId}' not found in Orbitstrap-things manifest.");

            string tempZip = Path.Combine(Path.GetTempPath(), $"orbitstrap-skybox-{skyboxId}.zip");
            string tempExtract = Path.Combine(Path.GetTempPath(), $"orbitstrap-skybox-{skyboxId}-extract");
            if (Directory.Exists(tempExtract))
                Directory.Delete(tempExtract, true);

            var data = await SkyboxHttpClient.GetByteArrayAsync(entry.Url);
        ...
        public static async Task ApplySkyboxAsync(string skyboxId, string modsFolder)
        {
            string src = await EnsureSkyboxDownloadedAsync(skyboxId);
