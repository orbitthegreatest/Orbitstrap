...
namespace Orbitstrap.RobloxInterfaces
{
    public static class Deployment
    {
        ...
        static Deployment()
        {
            SharedHttp.DefaultRequestHeaders.TryAddWithoutValidation("User-Agent", "OrbitstrapUpdater/2.0");
        ...
        private static async Task<T> SafeGetJson<T>(string url, int maxRetries = 4)
        {
            var rng = new Random();
            for (int attempt = 1; attempt <= maxRetries + 1; attempt++)
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    {
                        int delay = 1000 * (1 << (attempt - 1));
                        int jitter = (int)(delay * 0.2 * (rng.NextDouble() * 2 - 1));
                        delay = Math.Clamp(delay + jitter, 500, 20_000);
            ...
            throw new InvalidHTTPResponseException($"All retries exhausted for {url}");
        }
            ...
            DownloadForModGenerator(bool overwrite = false)
        {
            ...
            string tmp = Path.Combine(Path.GetTempPath(), "Orbitstrap");
