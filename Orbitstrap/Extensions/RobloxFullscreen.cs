...
public class RobloxFullscreen
{
    ...
    public static void WaitAndTriggerFullscreen()
    {
        ...
        string processName = Orbitstrap.App.RobloxPlayerAppName.Split('.')[0];
        Orbitstrap.App.Logger.WriteLine(LOG_IDENT, $"Waiting for {processName} to start and become visible...");
        ...
        while (sw.Elapsed.TotalSeconds < 60)
        {
            ...
            if (processes.Length > 0)
            {
                ...
                {
                    Orbitstrap.App.Logger.WriteLine(LOG_IDENT, "Found visible Roblox window, triggering Alt+Enter");

                    SetForegroundWindow(roblox.MainWindowHandle);
                    Thread.Sleep(500);

                    SendAltEnter();
                    Orbitstrap.App.Logger.WriteLine(LOG_IDENT, "Alt+Enter triggered");
                    return;
                }
            }
            Thread.Sleep(500);
        ...
        Orbitstrap.App.Logger.WriteLine(LOG_IDENT, "Timed out waiting for Roblox window");
    }
