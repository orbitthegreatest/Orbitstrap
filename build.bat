...
echo.
echo    gh auth login
echo    gh release create v3.7.5 "publish_output\Orbitstrap.exe" ^
echo        --repo orbitthegreatest/Orbitstrap ^
echo        --title "Orbitstrap v3.7.5" ^
echo        --notes "Fix: in-game resolution changer now reliably applies when joining a game (fixed a corrupted DEVMODE struct that could silently crash tracking). Added: Advanced resolution rules for multiple games. Changed: removed stale version number from the tray watcher menu."
echo.
