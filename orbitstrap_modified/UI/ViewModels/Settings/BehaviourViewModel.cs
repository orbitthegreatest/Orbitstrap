...
using static OrbitstrapRobloxSettingsManager;
...
namespace Orbitstrap.UI.ViewModels.Settings
{
    public class BehaviourViewModel : NotifyPropertyChangedViewModel
    {
        ...
        {
            var settings = new OrbitstrapRobloxSettings
            {
                MemoryCleanerIntervalSeconds =
                    SelectedMemoryCleanerInterval?.Seconds ?? 0
            };

            OrbitstrapRobloxSettingsManager.Save(settings);
        }
        ...
        public bool CleanerOrbitstrap
        {
            ...
            set
            {
                if (value && !CleanerItems.Contains("OrbitstrapLogs"))
                {
                    CleanerItems.Add("OrbitstrapLogs");
                    UpdateCleanerItems();
                }
                else if (!value && CleanerItems.Contains("OrbitstrapLogs"))
                {
                    CleanerItems.Remove("OrbitstrapLogs");
                    UpdateCleanerItems();
                }
                OnPropertyChanged(nameof(CleanerOrbitstrap));
            }
        }
