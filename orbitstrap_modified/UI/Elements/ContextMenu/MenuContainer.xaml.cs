...
using Orbitstrap.UI.Chat;
using Orbitstrap.UI.Elements.Crosshair;
using Orbitstrap.UI.Elements.FPS;
...
namespace Orbitstrap.UI.Elements.ContextMenu
{
    ...
    public partial class MenuContainer
    {
        ...
        private void MemoryTimer_Tick(object? sender, EventArgs e)
        {
            try
            {
                var orbitstrapProcess = Process.GetProcessesByName("Orbitstrap").FirstOrDefault();
                long orbitstrapMemory = orbitstrapProcess?.WorkingSet64 ?? 0;
        ...
        public void ActivityWatcher_OnGameJoin(object? sender, EventArgs e)
        {
            ...
            if (App.Settings.Prop.IngameChatDiscord)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    if (App.Current.Resources["DiscordChatOverlayWindow"]
                        is Orbitstrap.UI.Elements.Overlay.DiscordChatOverlayWindow existing)
                    {
                    ...
                    var discordOverlay = new Orbitstrap.UI.Elements.Overlay.DiscordChatOverlayWindow();
                    discordOverlay.Show();
            ...
            if (App.Settings.Prop.OverlaysEnabled)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    if (App.Current.Resources["OverlayWindow"] is Orbitstrap.UI.Elements.Overlay.OverlayWindow existing)
                    {
                        if (existing.IsLoaded)
                        {
                            return;
                    ...
                    var overlay = new Orbitstrap.UI.Elements.Overlay.OverlayWindow();
                    overlay.Show();
        ...
        public void ActivityWatcher_OnGameLeave(object? sender, EventArgs e)
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                ...
                if (App.Current.Resources["OverlayWindow"] is Orbitstrap.UI.Elements.Overlay.OverlayWindow overlay)
                {
                    overlay.Close();
                    App.Current.Resources.Remove("OverlayWindow");
                }

                if (App.Current.Resources["DiscordChatOverlayWindow"] is Orbitstrap.UI.Elements.Overlay.DiscordChatOverlayWindow discordOverlay)
                {
