    ...
    public class Bootstrapper
    {
        ...
        private async Task StartRoblox(CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                // ModInjector is launched after Roblox starts (needs PID for injection)
                HandleFullBright();
            ...
            CancellationToken ct)
        {
            ...
            try
            {
                ...
                // Launch ModInjector now that we have the Roblox PID for injection
                await Task.Run(() => LaunchModInjectorIfEnabled(_appPid), ct);
        ...
        private static void LaunchModInjectorIfEnabled(int robloxPid = 0)
        {
            const string LOG_IDENT = "Bootstrapper::LaunchModInjectorIfEnabled";
            try
            {
                if (!App.Settings.Prop.UseModInjector) return;

                // Candidate locations for ModInjector.exe, in priority order:
                // 1. Standard ModInjector install in LocalAppData
                // 2. Bundled copy next to Orbitstrap (Resources\ModInjector\)
                // 3. Copied to Orbitstrap's own data folder
                string localAppData = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
                string[] candidates = new[]
                {
                    Path.Combine(localAppData, "ModInjector", "ModInjector.exe"),
                    Path.Combine(Path.GetDirectoryName(Paths.Process) ?? AppContext.BaseDirectory, "Resources", "ModInjector", "ModInjector.exe"),
                    Path.Combine(Paths.Base, "ModInjector", "ModInjector.exe"),
                };

                string? modInjectorExe = candidates.FirstOrDefault(File.Exists);

                if (modInjectorExe is null)
                {
                    App.Logger.WriteLine(LOG_IDENT, "ModInjector.exe not found in any known location. Please install ModInjector first.");
                    return;
                }

                string modInjectorDir = Path.GetDirectoryName(modInjectorExe)!;
                App.Logger.WriteLine(LOG_IDENT, $"Found ModInjector at: {modInjectorExe}");

                // Check if already running
                var existing = Process.GetProcessesByName("ModInjector")
                    .FirstOrDefault(p => !p.HasExited);
                if (existing != null)
                {
                    App.Logger.WriteLine(LOG_IDENT, "ModInjector already running.");
                    return;
                }

                App.Logger.WriteLine(LOG_IDENT, $"Launching ModInjector targeting Roblox PID {robloxPid}");
                Process.Start(new ProcessStartInfo
                {
                    FileName = modInjectorExe,
                    Arguments = robloxPid > 0 ? robloxPid.ToString() : string.Empty,
                    WorkingDirectory = modInjectorDir,
                    UseShellExecute = false
                });

                // Give ModInjector time to inject before Roblox loads into the game
                Thread.Sleep(2000);
            }
            catch (Exception ex)
            {
                App.Logger.WriteLine(LOG_IDENT, $"ModInjector launch failed: {ex}");
            }
        }
